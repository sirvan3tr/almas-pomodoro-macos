...
final class PomodoroTimer {
    ...
    /// Begin a new countdown for `preset`. If a timer is already running
    ...
    func start(_ preset: Preset, now: Date = Date()) {
        invalidate()
        endDate = now.addingTimeInterval(TimeInterval(preset.seconds))
        state = .running(preset: preset, remaining: preset.seconds)
        scheduleTicker()
    }
    ...
    func tick(now: Date) {
        guard case .running(let preset, _) = state, let end = endDate else { return }
        let remaining = Int(ceil(end.timeIntervalSince(now)))
        if remaining <= 0 {
            invalidate()
            state = .finished(preset: preset)
            return
        }
        state = .running(preset: preset, remaining: remaining)
    }
